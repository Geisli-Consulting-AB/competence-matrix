...
export async function buildExperiencePage(
  ...
  if (experiences && experiences.length > 0) {
    ...
    experiences.forEach((exp, index) => {
      ...
      // Format date range
      const startDate = exp.startMonth ? `${exp.startMonth} ${exp.startYear}` : exp.startYear;
      const endDate = exp.ongoing 
        ? ('ongoing' in strings ? strings.ongoing : 'Ongoing')
        : (exp.endMonth ? `${exp.endMonth} ${exp.endYear}` : exp.endYear);
      ...
      doc.text(employerLines, m.leftPadding, y + (LAYOUT.SPACING.SECTION_HEADER * 0.8));
      ...
      y += Math.max(
        ...
        employerLines.length * (LAYOUT.FONT.SUBTITLE + 2) + (LAYOUT.SPACING.SECTION_HEADER * 0.8)
      ...
      if (exp.competences && exp.competences.length > 0) {
        ...
        y += competencesLines.length * (LAYOUT.FONT.SMALL + 1) + (LAYOUT.SPACING.LIST_ITEM / 2);
      } else {
