  ...
  if (educations && educations.length > 0) {
    ...
    educations.forEach((edu, index) => {
      ...
      if (edu.fieldOfStudy) {
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(LAYOUT.FONT.BODY);
        doc.text(edu.fieldOfStudy, rightColX, y + (LAYOUT.SPACING.HEADING_UNDERLINE / 2)); // Half of HEADING_UNDERLINE for medium spacing
      }
      
      // Description if available
      if (edu.description) {
        const descY = y + (edu.fieldOfStudy ? LAYOUT.SPACING.SECTION_HEADER : (LAYOUT.SPACING.HEADING_UNDERLINE / 2));
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(LAYOUT.FONT.BODY);
