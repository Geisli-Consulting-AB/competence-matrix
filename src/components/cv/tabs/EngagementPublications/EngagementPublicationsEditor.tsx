...
const EngagementPublicationsEditor: React.FC<EngagementPublicationsEditorProps> = ({ items = [], onChange }) => {
  ...
  const updateItem = <K extends keyof EngagementPublication>(index: number, field: K, value: EngagementPublication[K]) => {
    const updated = [...(items || [])];
      ...
      {(items || []).map((it, index) => (
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <IconButton aria-label="remove engagement/publication" onClick={() => removeItem(index)} sx={{ ml: 1 }}>
              <DeleteOutlineIcon />
            </IconButton>
            <IconButton
          ...
          <Collapse in={!!expandedById[it.id]} timeout="auto" unmountOnExit>
            ...
            <Box sx={{ mt: 1, ml: 0 }}>
              <TextField
                ...
                onChange={(e) => updateItem(index, 'locationOrPublication', e.target.value)}
                onBlur={(e) => updateItem(index, 'locationOrPublication', e.target.value)}
                margin="normal"
                variant="outlined"
              />
            </Box>
            ...
            <Box sx={{ mt: 1, ml: 0 }}>
              <TextField
                ...
                onBlur={(e) => updateItem(index, 'description', e.target.value)}
                margin="normal"
                variant="outlined"
                multiline
                rows={3}
