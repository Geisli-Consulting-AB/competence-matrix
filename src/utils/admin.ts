 ...
 */
export async function findUserByEmail(email: string): Promise<{ userId: string; email: string; displayName?: string } | null> {
  ...
  try {
    ...
    if (!snap1.empty) {
      ...
      return { userId: docSnap.id, email: data.email || targetEmail, displayName: data.displayName };
    }
    ...
    console.error('Error looking up user by email', { email: targetEmail, error });
    return null;
  }
}
